<?php
    ...
    $pauzeplaats = $_GET["pauzeplaats"] ?? -1;
    $booking = $_GET["booking"] ?? -1;

    if ($pauzeplaats === -1 || $booking === -1) {
    ...
    if (isset($_POST["Aanpassen"])) {
        // Status veranderen.
        $query = $connection->prepare("UPDATE pauzeplaatsen SET FKstatussenID = :statusID WHERE ID = :id");
        $query->bindParam("statusID", $_POST["status"]);
        $query->bindParam("id", $pauzeplaats);
        $query->execute();
        ...
        header("Location: index.php?page=Beheer/Bookings/Overnachtingen/Index&booking=$booking&Updated");
    }
?>